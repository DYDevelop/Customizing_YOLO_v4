...
def image_preprocess(image, target_size, gt_boxes=None):
    ih, iw    = target_size # 416 X 416
    h,  w, _  = image.shape # 600 X 600
    # gt_boxes = xmin, ymin, xmax, ymax, x1, y1, x2, y2
    scale = min(iw/w, ih/h) # 416/600 == 0.69
    nw, nh  = int(scale * w), int(scale * h) # 416, 416
    image_resized = cv2.resize(image, (nw, nh)) # 416 X 416 Resize
    ...
    if gt_boxes is None:
        ...
        gt_boxes[:, [4, 6]] = gt_boxes[:, [4, 6]] * scale + dw
        gt_boxes[:, [5, 7]] = gt_boxes[:, [5, 7]] * scale + dh
...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        score = bbox[4]
        junc =  np.array(bbox[5:9], dtype=np.int32)
        vector = np.array(bbox[9:11], dtype=np.float16)
        occu = bbox[11]
        ...
        if occu > 0.5:
            color = (0, 255, 255)
        cv2.line(image, (junc[0], junc[1]), (junc[2], junc[3]), color, 3)
        cv2.line(image, (junc[0], junc[1]), (np.int32(junc[0] + 50*vector[0]), np.int32(junc[1] + 50*vector[1])), color, 3)
        cv2.line(image, (junc[2], junc[3]), (np.int32(junc[2] + 50*vector[0]), np.int32(junc[3] + 50*vector[1])), color, 3)
...
def nms(bboxes, iou_threshold, sigma=0.3, method='nms'):
    ...
    """
    classes_in_img = list(set(bboxes[:, 12]))
    best_bboxes = []

    for cls in classes_in_img:
        cls_mask = (bboxes[:, 12] == cls)
...
def postprocess_boxes(pred_bbox, original_image, input_size, score_threshold):
    ...
    pred_junc = pred_bbox[:, 5:9] # x1, y1, x2, y2
    pred_vector = pred_bbox[:, 9:11]
    pred_occu = pred_bbox[:, 11]
    pred_prob = pred_bbox[:, 12:]
    ...
    pred_junc[:, 0::2] = 1.0 * (pred_junc[:, 0::2] - dw) / resize_ratio
    pred_junc[:, 1::2] = 1.0 * (pred_junc[:, 1::2] - dh) / resize_ratio
    ...
    coors, scores, juncs, vectors, occupancy, classes = pred_coor[mask], scores[mask], pred_junc[mask],  pred_vector[mask],  pred_occu[mask], classes[mask]

    return np.concatenate([coors, scores[:, np.newaxis], juncs, vectors, occupancy[:, np.newaxis], classes[:, np.newaxis]], axis=-1)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.5, iou_threshold=0.45, rectangle_colors=''):
    ...
    # for i, bbox in enumerate(bboxes):
    #     coor = np.array(bbox[:4], dtype=np.int32)
    #     image = cv2.rectangle(image, (coor[0], coor[1]), (coor[0]+40, coor[1]+40), (0, 255, 255))
    #     image = cv2.rectangle(image, (0 if coor[2]-40<0 else coor[2]-40, 
    #                                   0 if coor[3]-40<0 else coor[3]-40), (coor[2], coor[3]), (0, 255, 255))
