...
def YOLOv4(input_layer, NUM_CLASS):
    ...
    route_1 = conv
    conv = convolutional(conv, (3, 3, 128, 256))
    conv_sbbox = convolutional(conv, (1, 1, 256, 3 * (12 + NUM_CLASS)), activate=False, bn=False)
    ...
    route_2 = conv
    conv = convolutional(conv, (3, 3, 256, 512))
    conv_mbbox = convolutional(conv, (1, 1, 512, 3 * (12 + NUM_CLASS)), activate=False, bn=False)
    ...
    conv = convolutional(conv, (3, 3, 512, 1024))
    conv_lbbox = convolutional(conv, (1, 1, 1024, 3 * (12 + NUM_CLASS)), activate=False, bn=False)
...
def decode(conv_output, NUM_CLASS, i=0):
    ...
    conv_output = tf.reshape(conv_output, (batch_size, output_size, output_size, 3, 12 + NUM_CLASS))

    #conv_raw_dxdy = conv_output[:, :, :, :, 0:2] # offset of center position     
    #conv_raw_dwdh = conv_output[:, :, :, :, 2:4] # Prediction box length and width offset
    #conv_raw_conf = conv_output[:, :, :, :, 4:5] # confidence of the prediction box
    #conv_raw_coor = conv_output[:, :, :, :, 5: ] # x1, y1, x2, y2
    conv_raw_dxdy, conv_raw_dwdh, conv_raw_conf, conv_raw_dot1, conv_raw_dot2, conv_raw_vector, conv_raw_occu, conv_raw_prob = tf.split(conv_output, (2, 2, 1, 2, 2, 2, 1, NUM_CLASS), axis=-1)
    ...
    xy_grid = tf.meshgrid(tf.range(output_size), tf.range(output_size)) # output_size = 52, 26, 13
    xy_grid = tf.expand_dims(tf.stack(xy_grid, axis=-1), axis=2)  # [gx, gy, 1, 2]
    xy_grid = tf.tile(tf.expand_dims(xy_grid, axis=0), [batch_size, 1, 1, 3, 1])
    xy_grid = tf.cast(xy_grid, tf.float32) # x축과 y축으로 만든 Grid 형태 [batch_size, 52, 52, 3, 1], (0,0,0,0) -> (1,1,0,0) 뭐 이런식으로 좌표점 느낌
    
    #xy_grid = tf.concat([x[:, :, tf.newaxis], y[:, :, tf.newaxis]], axis=-1)
    #xy_grid = tf.tile(xy_grid[tf.newaxis, :, :, tf.newaxis, :], [batch_size, 1, 1, 3, 1])
    #xy_grid = tf.cast(xy_grid, tf.float32)
    # Calculate the center position of the prediction box:
    pred_xy = (tf.sigmoid(conv_raw_dxdy) + xy_grid) * STRIDES[i] # 최종 0 ~ 416 값으로 나옴
    ...
    # 이거 한번 돌려보기 lr -> 5 ~ 7
    # pred_dot1 = pred_xy + (tf.sigmoid(conv_raw_dot1) - tf.sigmoid(conv_raw_dxdy)) * pred_wh/2
    
    # pred_dot2 = pred_xy + (tf.sigmoid(conv_raw_dot2) - tf.sigmoid(conv_raw_dxdy)) * pred_wh/2
    
    # pred_dot1 = pred_xy + tf.tanh(conv_raw_dot1) * ANCHORS[i] * STRIDES[i] / 2
    
    # pred_dot2 = pred_xy + tf.tanh(conv_raw_dot2) * ANCHORS[i] * STRIDES[i] / 2
    
    # pred_dot1 = pred_xy + tf.tanh(conv_raw_dot1) * pred_wh 
    
    # pred_dot2 = pred_xy + tf.tanh(conv_raw_dot2) * pred_wh 
    
    pred_dot1 = (xy_grid + 0.5 + tf.tanh(conv_raw_dot1) * ANCHORS[i]) * STRIDES[i] # [8, 16, 32]
    
    pred_dot2 = (xy_grid + 0.5 + tf.tanh(conv_raw_dot2) * ANCHORS[i]) * STRIDES[i]
    
    pred_coor = tf.concat([pred_dot1, pred_dot2], axis=-1)
    
    # Conv을 통해 학습한 값으로 
    vector = tf.tanh(conv_raw_vector)
    
    length = tf.expand_dims(tf.sqrt(tf.pow(vector[:, :, :, :, 0], 2) + tf.pow(vector[:, :, :, :, 1], 2)), axis=-1)
    
    pred_vector = vector / length
    
    # 예측한 두 점으로 각도 구하기 None 값이 안들어감
    # alpha = tf.atan((pred_dot1[1] - pred_dot2[1]) / (pred_dot1[0] - pred_dot2[0])) * (180 / np.pi)
    
    # pred_angle = tf.where(alpha < 0, alpha + 270, alpha + 90)
    
    pred_occu = tf.sigmoid(conv_raw_occu)
    ...
    # pred_coor = tf.sigmoid(conv_raw_coor) # calculating the predicted probability category box object    
    # calculating the predicted probability category box object
    return tf.concat([pred_xywh, pred_conf, pred_coor, pred_vector, pred_occu, pred_prob], axis=-1)


def bbox_iou(boxes1, boxes2):
    boxes1_area = boxes1[..., 2] * boxes1[..., 3] + 0.000001
    boxes2_area = boxes2[..., 2] * boxes2[..., 3] + 0.000001
    ...
    inter_area = inter_section[..., 0] * inter_section[..., 1] + 0.000001
    union_area = boxes1_area + boxes2_area - inter_area

    return 1.0 * inter_area / union_area

def bbox_giou(boxes1, boxes2):
    boxes1 = tf.concat([boxes1[..., :2] - boxes1[..., 2:] * 0.5,  # xmin, ymin 만들기
                        boxes1[..., :2] + boxes1[..., 2:] * 0.5], axis=-1) # xmax, ymax 만들기
    boxes2 = tf.concat([boxes2[..., :2] - boxes2[..., 2:] * 0.5, # GT_bboxes
                        boxes2[..., :2] + boxes2[..., 2:] * 0.5], axis=-1)
    # boxes 상태 = [xmin, ymin, xmax, ymax]
    # 혹시 잘못됐으면 고쳐주는 곳
    ...
    boxes1_area = (boxes1[..., 2] - boxes1[..., 0]) * (boxes1[..., 3] - boxes1[..., 1]) + 0.000001 # 박스 넓이 구하기
    boxes2_area = (boxes2[..., 2] - boxes2[..., 0]) * (boxes2[..., 3] - boxes2[..., 1]) + 0.000001
    left_up = tf.maximum(boxes1[..., :2], boxes2[..., :2])
    right_down = tf.minimum(boxes1[..., 2:], boxes2[..., 2:])

    inter_section = tf.maximum(right_down - left_up, 0.0) # 좌상단->우하단 까지의 길이 차
    inter_area = inter_section[..., 0] * inter_section[..., 1] + 0.000001
    union_area = boxes1_area + boxes2_area - inter_area 
    ...
    # Calculate the area of the smallest closed convex surface 
...
def compute_loss(pred, conv, label, bboxes, i=0, CLASSES=YOLO_COCO_CLASSES):
    ...
    conv = tf.reshape(conv, (batch_size, output_size, output_size, 3, 12 + NUM_CLASS))

    conv_raw_conf = conv[:, :, :, :, 4:5]
    conv_raw_occu = conv[:, :, :, :, 11]
    conv_raw_prob = conv[:, :, :, :, 12:]
    # conv_raw_coor = conv[:, :, :, :, 5:]
    # print(np.shape(pred)) #(4, 52, 52, 3, 12)
    
    # pred(None, 52, 52, 3, 10) -> [pred_xywh, pred_conf, pred_coor]
    pred_xywh     = pred[:, :, :, :, 0:4]
    pred_conf     = pred[:, :, :, :, 4:5]
    pred_coor     = pred[:, :, :, :, 5:9]
    pred_vector   = pred[:, :, :, :, 9:11]
    pred_occu     = pred[:, :, :, :, 11]
    
    # print(np.shape(label)) #(4, 52, 52, 3, 10)
    # label[i][yind, xind, iou_mask, :]
    label_xywh    = label[:, :, :, :, 0:4]
    respond_bbox  = label[:, :, :, :, 4:5]  # confidence
    label_coor    = label[:, :, :, :, 5:9]
    label_vector  = label[:, :, :, :, 9:11]
    label_occu    = label[:, :, :, :, 11]
    label_prob    = label[:, :, :, :, 12:]
    ...
    bbox_loss_scale = 2.0 - 1.0 * label_xywh[:, :, :, :, 2:3] * label_xywh[:, :, :, :, 3:4] / (input_size ** 2) # 2 - w * h / 416^2
    ...
    conf_focal = tf.pow(respond_bbox - pred_conf, 2) # (1.0 - pred_conf)^2
    ...
    # dot1_loss = tf.abs(pred_coor[:, :, :, :, 0:2] - label_coor[:, :, :, :, 0:2] - xy_grid * STRIDES[i])
    # dot2_loss = tf.abs(pred_coor[:, :, :, :, 2:] - label_coor[:, :, :, :, 2:] - xy_grid * STRIDES[i])
    # coor_loss = respond_bbox * tf.concat([dot1_loss, dot2_loss], axis=-1)/1000
    
    # coor_loss = respond_bbox * tf.pow(pred_coor - label_coor, 2)
    
    # /100 -> AP 66%, /416 -> 77%
    coor_loss = respond_bbox * tf.sqrt(tf.pow(pred_coor - label_coor, 2))
    
    dot_product = tf.expand_dims(pred_vector[:, :, :, :, 0] * label_vector[:, :, :, :, 0] + pred_vector[:, :, :, :, 1] * label_vector[:, :, :, :, 1], axis=-1)
    
    pred_vector_length = tf.expand_dims(tf.sqrt(tf.pow(pred_vector[:, :, :, :, 0], 2) + tf.pow(pred_vector[:, :, :, :, 1], 2)), axis=-1)
    
    label_vector_length = tf.expand_dims(tf.sqrt(tf.pow(label_vector[:, :, :, :, 0], 2) + tf.pow(label_vector[:, :, :, :, 1], 2)), axis=-1)
    
    angle = tf.acos(dot_product / (pred_vector_length * label_vector_length + 0.0001))
    
    angle_loss = respond_bbox * angle
    
    # angle_loss = respond_bbox * tf.sqrt(tf.pow((label_coor[:, :, :, :, 0:2] + pred_vector) - (label_coor[:, :, :, :, 0:2] + label_vector), 2))
    
    occu_loss = respond_bbox * tf.expand_dims(tf.nn.sigmoid_cross_entropy_with_logits(labels=label_occu, logits=conv_raw_occu), axis=-1)
    
    prob_loss = respond_bbox * tf.nn.sigmoid_cross_entropy_with_logits(labels=label_prob, logits=conv_raw_prob)
    
    giou_loss = tf.reduce_mean(tf.reduce_sum(giou_loss, axis=[1,2,3,4]))
    conf_loss = tf.reduce_mean(tf.reduce_sum(conf_loss, axis=[1,2,3,4]))
    coor_loss = tf.reduce_mean(tf.reduce_sum(coor_loss, axis=[1,2,3,4])) / 100
    angle_loss = tf.reduce_mean(tf.reduce_sum(angle_loss, axis=[1,2,3,4]))
    occu_loss = tf.reduce_mean(tf.reduce_sum(occu_loss, axis=[1,2,3,4]))
    prob_loss = tf.reduce_mean(tf.reduce_sum(prob_loss, axis=[1,2,3,4]))
    
    return giou_loss, conf_loss, coor_loss, angle_loss, occu_loss, prob_loss
